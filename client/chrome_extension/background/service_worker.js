...
/**
 * 選択テキストまたはリンクを x-keeper にダウンロードキューとして送信できるよう
 * 右クリックメニューに「x-keeper でダウンロード」を追加する。
 *
 * - contexts: ['selection'] 選択テキスト (URL / マグネットリンク) を右クリックした場合
 * - contexts: ['link']      リンクを右クリックした場合
 *
 * Manifest V3 の Service Worker は onInstalled 以外でも contextMenus を登録できるが、
 * removeAll() → create() で冪等にする。
 */
function registerContextMenus() {
  chrome.contextMenus.removeAll(() => {
    chrome.contextMenus.create({
      id: 'xkeeper-queue-selection',
      title: 'x-keeper でダウンロード',
...
/** 選択テキストまたはリンク URL が対応フォーマットか確認する */
...
chrome.contextMenus.onClicked.addListener(async (info) => {
  // 選択テキストを優先、なければリンク URL を使用
  const raw = (info.selectionText || info.linkUrl || '').trim();
  if (!raw) return;

  // 選択テキストが複数行のとき最初の有効な URL/マグネットを抽出する
  const candidate = raw.split(/\s+/).find(isSupportedUrl) || raw;
  if (!isSupportedUrl(candidate)) {
    console.warn('[x-keeper] コンテキストメニュー: 対応外のテキスト:', candidate);
    return;
  }

  try {
    const data = await postUrls([candidate]);
    await flushQueue();
    await appendResultLog({
      ts: new Date().toISOString(),
      status: 'queued',
    ...
    pollDownloadedIds();
    pollDownloadedUrls();
    console.log('[x-keeper] コンテキストメニュー: キュー送信成功:', candidate);
  } catch (e) {
    await enqueueOffline(candidate);
    await appendResultLog({
      ts: new Date().toISOString(),
      status: 'offline',
      urls: [candidate],
    });
    console.warn('[x-keeper] コンテキストメニュー: オフラインキューに保存:', candidate);
  }
});
