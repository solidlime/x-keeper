...
X (Twitter) / Pixiv / Imgur などのメディアを自動ダウンロード・保存する Web アプリケーション。
...
- **複数サービス対応**: Pixiv / Imgur にも対応
- **重複防止**: ダウンロード済み tweet ID を記録。再投稿されても重複ダウンロードしない
- **Web UI**: Flask ベースのモダン UI。ギャラリー閲覧・ログ確認・リトライ管理が可能
...
X / Pixiv ページに自動的にダウンロードボタンを注入し、直接 URL をキューに投入できる。サーバー未接続時はオフラインキューに保存され、接続回復後に自動送信される。
...
4. **拡張機能を読み込む**
   ...
   - `client/chrome_extension/` フォルダを選択

5. **拡張機能を設定**
   - Chrome ツールバーの拡張機能アイコンをクリック
   - 「オプション」または「詳細」を開く
   - サーバー URL を設定: `http://localhost:8989`

### 機能

- **X.com / Pixiv ページへのボタン注入**: ページにダウンロードボタンが自動表示される
- **ダウンロード済みバッジ**: リアルタイムで既ダウンロード件数を表示
- **オフラインキュー**: サーバー未接続時もキューに積めて、後で送信可能
- **キュー管理**: ポップアップからキュー一覧の確認・個別キャンセル・全クリアが可能
...
│   ├── image_downloader.py       # gallery-dl ラッパー・ダウンロード処理
│   ├── twitter_client.py         # gallery-dl --dump-json でスレッド取得
│   ├── log_store.py              # JSON 永続化ログ・APIキュー管理
│   └── web_setup.py              # Flask アプリ・Web UI ルート定義
│
├── client/
...
│   │   ├── user-123456789-01.jpg
│   │   └── ...
│   ├── _download_log.json        # ダウンロードログ (最大 500 件)
│   ├── _retry_queue.json         # リトライキュー
│   ├── _downloaded_ids.json      # ダウンロード済み tweet ID リスト
│   └── _api_queue.json           # API キュー
│
├── .env                          # 環境変数設定ファイル (要作成)
...
以下のファイルは `{SAVE_PATH}` 下に JSON 形式で自動保存される：

| ファイル | 説明 |
|---|---|
| `_download_log.json` | ダウンロード履歴・失敗ログ (最大 500 件保持) |
| `_retry_queue.json` | リトライキュー |
| `_downloaded_ids.json` | ダウンロード済み tweet ID (重複防止用) |
| `_api_queue.json` | Chrome 拡張・Android アプリから投入された URL キュー |
...
2. LogStore が URL を `_api_queue.json` に保存
...
5. ダウンロード結果を LogStore に記録
   - 成功時: `_downloaded_ids.json` / `_api_queue.json` を更新
   - 失敗時: ログに失敗情報を記録

### 重複防止メカニズム

- **tweet ID ベース** (X): `_downloaded_ids.json` に tweet ID を永続保存。同じ tweet ID は 2 度ダウンロードしない
- **URL ベース** (Pixiv / Imgur): URL をそのまま `_downloaded_ids.json` に記録
...
| `gallery-dl` | メディアダウンロード (subprocess) |
| `aiofiles` | 非同期ファイル操作 |
