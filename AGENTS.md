...
Chrome 拡張・Android アプリから URL を受け取り・gallery-dl / yt-dlp で保存。Flask Web UI でギャラリー閲覧・設定管理・統計確認。
...
- `gallery-dl` を subprocess で実行して X/Pixiv/Imgur メディアをダウンロード
- `yt-dlp` を subprocess で実行して YouTube/TikTok/NicoNico 動画をダウンロード (F1)
- Flask Web UI でギャラリー閲覧・リトライ・ログ管理・ストレージ統計 (F3)
- ログ・キュー・重複防止データは SQLite (`xkeeper.db`) に永続化 (F5)

**技術スタック**:
- Python 3.11+、Flask、pydantic-settings
- gallery-dl (非公式 API、Twitter API キー不要)
- yt-dlp (YouTube / TikTok / NicoNico)
- Docker + docker-compose 推奨
- pytest + pytest-mock でテスト (85件)

**ファイル構成**:
- `src/main.py`: エントリーポイント・asyncio ループ・Flask デーモン起動
- `src/image_downloader.py`: `gallery-dl` / `yt-dlp` ラッパー
- `src/patterns.py`: URL 判定 regex の一元管理 (X / Pixiv / Imgur / `YT_DLP_URL_PATTERN`)
- `src/log_store.py`: SQLite 永続化ログ・リトライキュー管理・ストレージ統計
- `src/web_setup.py`: `src/web/` パッケージへの薄いシム (後方互換)
- `src/web/__init__.py`: Flask app + Blueprint 登録
- `src/web/routes/`: setup / gallery / api / media / admin の Blueprint モジュール
