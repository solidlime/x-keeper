...
Discord Bot で監視チャンネルの URL を自動検出・gallery-dl で保存。Flask Web UI でギャラリー閲覧・設定管理。
...
**主要機能**:
- Discord Bot が監視チャンネルをスキャンし X/Pixiv/Imgur URL を自動検出
- `gallery-dl` を subprocess で実行してメディアをダウンロード
- Flask Web UI でギャラリー閲覧・リトライ・ログ管理
- Chrome 拡張・Flutter Android アプリで外部から URL を投入可能

**技術スタック**:
- Python 3.11+、discord.py、Flask、pydantic-settings
- gallery-dl (非公式 API、Twitter API キー不要)
- Docker + docker-compose 推奨

**ファイル構成**:
- `src/main.py`: エントリーポイント・asyncio ループ・Flask デーモン起動
- `src/discord_bot.py`: Discord Bot 実装 (チャンネル監視・メッセージ処理)
- `src/image_downloader.py`: `gallery-dl` ラッパー
- `src/twitter_client.py`: Twitter スレッド遡り処理
